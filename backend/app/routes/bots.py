...
def toggle_bot_subscription(
    ...
    # Get user's subscription limits based on plan
    sub = db.query(Subscription).filter(Subscription.user_id == current_user.id).first()
    PLAN_BOT_LIMITS = {"starter": 1, "pro": 3, "elite": 99}
    max_bots = PLAN_BOT_LIMITS.get(sub.plan, 2) if sub and sub.status == "active" else 2
