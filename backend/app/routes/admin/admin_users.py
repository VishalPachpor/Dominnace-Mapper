...
def list_users(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    for u in users:
        # Aggregate stats (simplistic for now, can be optimized with join)
        trade_stats = db.query(
            func.count(Position.id).label("count"),
            func.sum(Position.pnl).label("pnl")
        ).filter(Position.user_id == u.id, Position.status == "closed").first()
        ...
        results.append({
            ...
            "status": u.status,
            "total_trades": trade_stats.count or 0,
            "total_pnl": round(float(trade_stats.pnl or 0), 2),
